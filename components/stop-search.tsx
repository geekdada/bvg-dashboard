...
export default function StopSearch() {
  ...
  return (
    <div className="relative mb-6">
      ...
      {isSearching && (
        <Card className="absolute z-10 w-full mt-1 shadow-lg max-h-80 overflow-auto">
              ...
              <ul id="search-results" ref={resultsRef} className="py-1" role="listbox">
                {results.map((stop, index) => (
                  <li
                    ...
                    className={`${selectedIndex === index ? "bg-gray-100 dark:bg-gray-800" : ""} cursor-pointer`}
                    onMouseEnter={() => setSelectedIndex(index)}
                    onClick={() => navigateToStop(stop.id)}
                  >
                    <div className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800">
                      <div className="font-medium">{stop.name}</div>
                      <div className="text-xs text-gray-500">ID: {stop.id}</div>
                    </div>
                  </li>
                ))}
              </ul>
